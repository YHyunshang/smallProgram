...
interface Props {
  cartWidth: number // 购物车图标宽度
  cartHeight: number // 购物车图标高度
  refreshGoodsList: Function // 刷新商品列表的方法
  goodsItem: {
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      animatedValue: new Animated.Value(
        props.goodsItem.productNum <= 0 ? 0 : 1
      ),
    }
  }
  static defaultProps = {
    ...
    return (
      <View style={styles.container}>
        <View style={styles.cartWrapper}>
          <TouchableOpacity activeOpacity={0.9} onPress={this.handleMinCart}>
            <Animated.View style={{ transform: [{ translateX }] }}>
              <Animated.Image
                style={{
                  width: cartWidth,
                  height: cartHeight,
                  transform: [{ rotate: minPositiveZ }],
                }}
                source={minusCircle}
              />
            </Animated.View>
          ...
          <Animated.Text
            style={[
              styles.numberText,
              { transform: [{ translateX: cartNumberTranslateX }] },
            ]}
            numberOfLines={1}
          >
          ...
          <TouchableOpacity activeOpacity={0.9} onPress={this.handleAddCart}>
            <Animated.Image
              style={[
                { width: cartWidth, height: cartHeight },
                { transform: [{ rotate: rotatePlus }] },
              ]}
              source={
                cartNumber >= goodsItem.stockQuantity
                  ? plusCircleDisabled
                  : plusCircle
              }
            />
