...
// 日期字符串转换为中文字符串
const datetimeStr2CNStr = (date: string) =>
  date ? dayjs(date).format('MM月DD日HH:mm') : ''
// 日期字符串转换为时间戳（毫秒）
const datetimeStr2Ts = (date: string) => (date ? dayjs(date).valueOf() : null)
// 日期字符串转中文
const dateStr2CNStr = (date: string) =>
  date ? dayjs(date).format('MM月DD日') : ''
// 时间字符串转时:分
const timeStr2Hm = (time: string) =>
  time
    ? time
        .split(':')
        .slice(0, 2)
...
const ProductSection: React.FunctionComponent<ProductSectionProps> = ({
  ...
  const subTitle = loading
    ...
    : preSaleData.activityProductBrief
    ...
    <View>
      ...
      <View style={[styles.section, styles.mainInfoBox, styles.ph15]}>
        <Text style={styles.h1}>
          {loading ? initialData.name : preSaleData.productName}
        </Text>
        {!!subTitle && <Text style={styles.h5}>{subTitle}</Text>}

        <View style={styles.preOrderRow}>
          <Text style={styles.normalText}>
            上市价 ¥
            {transPenny(
              loading ? initialData.slashedPrice : preSaleData.onSalePrice
            )}
          </Text>
          <View style={styles.preOrderCountBox}>
            <Text style={styles.preOrderCountText}>
              {preSaleData.reserveNumber || 0}人已预订
            </Text>
          </View>
        </View>
        ...
        <View style={styles.productPropertyRow}>
          ...
          {!!detailData.originPlace && (
            <View
              style={[
                styles.productPropertyItem,
                styles.productPropertyLastItem,
              ]}
            >
              <FastImage
                style={styles.productPropertyIcon}
      ...
      <View style={styles.section}>
        <View style={styles.orderInfoRow}>
          <View style={styles.orderInfoItem}>
            ...
            <View style={styles.orderInfoTextBox}>
              ...
              <Text style={styles.orderInfoText}>
                {datetimeStr2CNStr(preSaleData.activityBeginDate)}至
                {datetimeStr2CNStr(preSaleData.activityEndDate)}
              </Text>
            </View>
          </View>
          ...
          <View style={styles.orderInfoItem}>
            ...
            <View style={styles.orderInfoTextBox}>
              ...
              <Text style={styles.orderInfoText}>
                {dateStr2CNStr(preSaleData.selfDeliveryBeginTime)}至
                {dateStr2CNStr(preSaleData.selfDeliveryEndTime)}
              </Text>
              <Text style={styles.orderInfoText}>
                每天{timeStr2Hm(preSaleData.selfDeliveryBeginTimeSlot)}~
                {timeStr2Hm(preSaleData.selfDeliveryEndTimeSlot)}
              </Text>
            </View>
          </View>
        </View>
        {!!preSaleData.supplementExplain && (
          <View style={[styles.orderAddOnInfoRow, styles.ph15]}>
            <Text style={styles.orderAddOnText}>
              补充说明：{preSaleData.supplementExplain}
            </Text>
          </View>
        )}
      </View>
