  ...
  return (
    <>
      ...
      {productType === ProductType.LimitTimeBuy ? (
        ...
        <Normal productData={product} initialData={initialData} />
      )}
      ...
      {[2, 3, 6].includes(orderPromotion.promotionType) && (
        ...
        >
          <ActivityNavigator
            tag={getTagName(
              orderPromotion.promotionType,
              orderPromotion.ruleType
            )}
            code={orderPromotion.promotionCode}
            title={`指定商品${
              orderPromotion.labels.length > 0
                ? orderPromotion.labels.join('，')
                : '促销'
            }`}
          />
        </View>
      ...
      {similarProducts.length > 0 && productType !== ProductType.PreSale ? (
        ...
        <View
          style={{ height: productType === ProductType.PreSale ? 10 : 15 }}
        />
      )}
    </>
  )
