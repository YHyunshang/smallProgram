 ...
 * @LastEditTime : 2020-01-14 15:04:49
 */
...
interface State {
  isShowMore: boolean // 默认收起状态
  qualificationInfo: {
    year: number // 查询积分的年份
    yearIntegralList: {
      integralDate: string // 换购日期
      quantity: number // 换购数量
    }[] // 用户换购记录
    availableIntegral: number // 可用积分
    yearTotalNumber: number // 年度最多购买数量
    yearAvailableQuantity: number // 年度剩余购买数量
  }
}

export default class QualificationsQuery extends React.Component<Props, State> {
  loadingRef: any
  constructor(props) {
    super(props)
    this.state = {
  ...
  render() {
    ...
    const buyListWrapper =
      newBuyList && newBuyList.length > 0 ? (
        newBuyList.map((item, index) => (
          <View
            style={[styles.buyItem, { marginTop: index === 0 ? 10 : 0 }]}
            key={index}
          >
            <Text style={styles.monthInfo}>{item.integralDate}</Text>
            <Text style={styles.numberInfo}>
              换购<Text style={styles.normsInfo}>{item.quantity}瓶</Text>
            </Text>
          </View>
        ))
      ) : (
      ...
      <View style={styles.container}>
        ...
        >
          <View style={styles.topBanner}>
            ...
            <Text style={styles.currentPointValue}>
              {qualificationInfo.availableIntegral
                ? qualificationInfo.availableIntegral
                : 0}
            </Text>
          ...
          <View style={styles.centerBanner}>
            <View style={styles.monthCountWrapper}>
              <View style={styles.monthCountHeight}>
                ...
                <Text style={styles.totalNumber}>
                  总计{qualificationInfo.yearTotalNumber}瓶
                </Text>
              </View>
              <View>
            ...
            <View style={styles.yearWrapper}>
              <View style={styles.yearItem}>
                ...
                <View style={styles.yearBox}>
                  <Text style={styles.yearNumber}>
                    {qualificationInfo.yearAvailableQuantity}
                  </Text>
                </View>
              ...
              <View style={styles.limitWrapper}>
                {qualificationInfo &&
                  qualificationInfo.yearAvailableQuantity == 0 && (
